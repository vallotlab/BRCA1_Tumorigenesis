...
writexl::write_xlsx(mylist, paste0(DEdir,"/TOP_DEG_P16_vsBOTH_LP_AVD.xlsx"))
```
...
comph= FindMarkers(epith, ident.1 = "P16+ Pre-lesional", ident.2 = c("LP","Avd"), only.pos = T, logfc.threshold = 0.5) %>% dplyr::filter( p_val_adj < 0.05)
comph$gene= rownames(comph)
comph= comph  %>% arrange(desc(avg_log2FC))
############
compd= FindMarkers(epith, ident.2 = "P16+ Pre-lesional", ident.1 = c("LP","Avd"), only.pos = T, logfc.threshold = 0.8)
...
names(mylist)= c("UP_P16_vsLPAvd", "PEA_C2_UP_P16_vsLPAvd","DN_P16_vsLPAvd", "PEA_C2_P16_vsLPAvd" )
writexl::write_xlsx(mylist, paste0(DEdir,"/C2_Msig_DEG_P16_LP_Avd.xlsx"))
```
...
```{r}
Idents(epith)="subtype" #make sure the subtype is set as Default Ident Or precise group.by="subtype
res= FindMarkers(epith, ident.1 = "P16+ Pre-lesional", ident.2 = c("Basal", "LP", "Avd"), group.by="subtype",only.pos = F, logfc.threshold = 0.1) 
writexl::write_xlsx(res, paste0(DEdir, "/DEG_P16_vs_LPAvdBasal.xlsx"))
```
...
pdf(file = paste0(DEdir, "/VolcanoPlot_DEG_P16_avdLP_Basal_WOLegendTopFC1_2.pdf"),width = 10, height = 8)
...
DefaultAssay(epith)="RNA"
comph= FindMarkers(epith, ident.1 = "P16+ Pre-lesional", ident.2 = c("LP","Avd", "Basal"), only.pos = T, logfc.threshold = 0.5) %>% dplyr::filter( p_val_adj < 0.05) %>% rownames_to_column("gene")

comph= comph  %>% arrange(desc(avg_log2FC))
############
compd= FindMarkers(epith, ident.2 = "P16+ Pre-lesional", ident.1 = c("LP","Avd", "Basal"), only.pos = T, logfc.threshold = 0.8)  %>% rownames_to_column("gene") %>% filter( p_val_adj < 0.05)
compd= compd  %>% arrange(desc(g_log2FC))
##########
m_t2g <- msigdbr(species = "Mus musculus", category = "H") %>% dplyr::select(gs_name, gene_symbol) %>% as.data.frame()
##
...
#############
mylst= list(comph, pag, compd, dag)
names(mylist)= c("UP_P16_vsLPAvdBasal", "PEA_C2_UP_P16_vsLPAvdBasal","DN_P16_vsLPAvdBasal", "PEA_C2_P16_vsLPAvdBasal" )
writexl::write_xlsx(mylist, paste0(DEdir,"/C2_Msig_DEG_P16_LP_Avd_Basal.xlsx"))
