...
topgenes= topgenes %>% dplyr::filter(!gene  %in% grep("^mt-", topgenes$gene, value = T)) %>% dplyr::filter(!gene  %in% grep("^Gm", topgenes$gene, value = T))  %>% dplyr::filter(!gene  %in% grep("^AC[0-9]{1,6}", lmarkers$gene, value = T)) %>% arrange(desc(avg_log2FC)) %>% distinct(gene,.keep_all = TRUE)

#####rank per foldchange:
topgenes = topgenes %>% group_by(cluster) %>% arrange(desc(avg_log2FC), .by_group = T) %>% dplyr::slice(1:50, .preserve = T) 
##
...
for (i in seq(1:nrow(topgenes))){
  if (length(intersect(c("Krt5","Acta2", "Krt14"),unlist(strsplit(topgenes$gene[i], split = ", ")))) > 2){
      topgenes$name[i]= "Basal"
  }
  if (length(intersect(c("Csn2", "Csn1s1", "Lalba", "Ltf"),unlist(strsplit(topgenes$gene[i], split = ", ")))) > 2){
    topgenes$name[i]= "Avd"
  }
  ...
  if (length(intersect(c("Ccnd2","Mki67", "Lars2"),unlist(strsplit(topgenes$gene[i], split = ", ")))) >= 2){
      topgenes$name[i]= paste0("Mki67+ " ,topgenes$tumsize[i])
  }
  
  if( topgenes$tumsize[i]=="CTRL_PreCancerous"){
    topgenes$name[i]= "P16+ Pre-lesional"
  }
 
  else if(is.na(topgenes$name[i]) ){
    topgenes$name[i]=paste0(unlist(strsplit(topgenes$gene[i], split = ", "))[1],"+ ", topgenes$tumsize[i])
  }
}
...
write.csv(topgenes,paste0(clustDir, "/Epithelial_Topgenes_Cluster_AutomatiAnnotation.csv") )
rm(nbr)
gc()
```
...
```{r}
m <- epith@meta.data %>% dplyr::filter(subtype== "P16+ Pre-lesional")
m= m[, c(9,21)] %>% as.data.frame()
###
...
dt4$Var2= factor(dt4$Var2, levels = c("Basal", "H-S", "LP", "Avd","P16+ Pre-lesional","ST","MT","LT"))
