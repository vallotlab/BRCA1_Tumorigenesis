...
p16= FindMarkers(seu, ident.1 = "P16+ Pre-lesional", ident.2 = "LP", only.pos = TRUE, logfc.threshold = 0.8, min.diff.pct = 0.2) %>% dplyr::filter(p_val_adj<0.05 )  %>% tibble::rownames_to_column("gene")
##---------------------------####
...
writexl::write_xlsx(dt, paste0(cheaDir, "/Chea3_OverexpressedGenes_P16cyclingPop.xlsx"))
```

Work on the score:
```{r}

dt$Top=ifelse(dt$Score <= 54.59, "Top", "No")

pdf(paste0(cheaDir, "/Fig1_Plot_Rank_logScore.pdf"))
...
tf.expressiona= PrctCellExpringGene(seu, genes= top_tf, group.by = "all" )
tf.expressiona$rank= seq(1, nrow(tf.expressiona))
df= tf.expression %>% filter(Feature== "P16+ Pre-lesional")
...
writexl::write_xlsx(l,paste0(cheaDir, "/Chea3_TopTF_P16Cycling.xlsx"))
```
...
pdf(paste0(outputDir, "/Barplot_Top30TF_P16cluster.pdf"))
...
writexl::write_xlsx(top_TF, paste0(cheaDir, "/Top_TF_PretumoralCells_ExpressionPct.xlsx"))
...
```{r}
pdf(paste0(cheaDir, "/DotPlot_Top30Genes_CellProp_Rank_Score.pdf"))
p <- top_TF %>% ggplot(aes(y= log(Score), x=Rank, size= Cell_proportion, color= Cell_proportion)) + theme_bw() + geom_hline(yintercept = 4.2, color= "red")+
  geom_text_repel(
    data = subset(top_TF, Score <= 54.59),
    aes(label = TF),
    size = 3,
    box.padding = unit(0.35, "lines"),
...
```{r}
pdf(paste0(cheaDir, "/Fig2_DotPlot_Top30Genes_CellProp_FC_Rank_Score.pdf"))
