...
```{r}
gene.sets <- list(p16UP_signature =p16d[which(p16d$avg_log2FC > 0.8),1], EMT= toupper(emtg), Apoptosis=toupper(apoptog) )
		#	p16Down_signature = p16d[which(p16d$avg_log2FC < (-0.8)),1])
#gene.sets <- list(p16UP_signature =p16d[which(p16d$avg_log2FC > 0.8),1])
scores <- ScoreSignatures_UCell(normalized_counts, features=gene.sets) %>% as.data.frame()
head(scores)
scores$sample=rownames(scores)
###

#scores=scores[ which(rownames(scores) %in% tcga_br$Sample.ID),] %>% as.data.frame()
